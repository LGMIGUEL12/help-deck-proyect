  ...
  script: [
    {
      ...
      async: true,
      defer: true,
      onload: 'window.googleScriptLoaded = true'
    }
  ]
})
...
const { loginSimple, loginWithGoogle } = useAuth()
  ...
  const initGoogle = () => {
    if (window.google && window.google.accounts) {
      try {
        ...
        const container = document.getElementById('googleSignUpButtonContainer')
        if (container) {
...
const handleGoogleCallback = async (response) => {
  ...
  try {
    ...
    if (result.success) {
      // Redirigir a tickets después del registro con Google
      await navigateTo('/tickets')
    } else {
      alert(result.error || 'Error al autenticar con Google')
    }
  } catch (error) {
    console.error('Error en Google sign up:', error)
    alert('Error de conexión con Google. Por favor intenta de nuevo.')
  } finally {
