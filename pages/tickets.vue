        ...
        <ul class="space-y-2">
          ...
          <li>
            <NuxtLink to="#"
              class="flex items-center rounded-lg hover:text-white transition-colors hover:bg-[#5a7a67] text-gray-300"
              :class="sidebarVisible ? 'px-3 py-2' : 'px-2 py-2 justify-center'"
              :title="!sidebarVisible ? 'Configuración' : ''">
              <UIcon name="i-heroicons-cog-6-tooth" class="w-5 h-5" :class="sidebarVisible ? 'mr-3' : ''" />
...
// Computed property to show only current user's tickets
const userTickets = computed(() => {
  return allTickets.value.filter(ticket => ticket.userId === user.value?.id)
})
